...
const Navbar = () => {
  ...
  return (
    <div className="text-white/70 pt-6 fixed top-0 left-0 w-full z-40 bg-black/90">
      <div className="hidden md:flex items-center px-4 py-2 mx-auto max-w-[400px]">
        <ul className="flex flex-row p-4 space-x-8">
          {navLinks.map((link, index) => (
            <li key={index}>
              <a href={link.path} className={link.title === "About" ? "group relative" : ""}>
                {link.title}
              </a>
            </li>
          ))}
      ...
      <div onClick={toggleNav} className="md:hidden absolute top-5 left-5 border rounded text-white/70 border-white/70 p-2 z-50 bg-orange-300">
